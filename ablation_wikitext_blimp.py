...
against SwiGLU baseline on language modeling with BLiMP evaluation.
...
Evaluation: BLiMP benchmark via lm_eval after each epoch
"""
...
class TrainingConfig:
    ...
    eval_blimp: bool = True
    blimp_limit: int = 100  # Samples per BLiMP subtask (None for full)
    ...
    seed: int = 42
    output_dir: str = 'results/wikitext_blimp_ablation'
...
class LMEvalWrapper(HFLM):
    ...
    def __init__(self, model, tokenizer, batch_size=1, device='cuda'):
        ...
        # Required attributes for distributed evaluation
        self._rank = 0
        self._world_size = 1
    ...
    def _model_generate(self, context, max_length, stop, **kwargs):
        """Generate tokens (not used for BLiMP)."""
    ...
    def loglikelihood(self, requests):
        ...
        for context, continuation in [req.args for req in requests]:
            ...
            if len(all_tokens) > self.max_length:
                ...
                ctx_tokens = all_tokens[:-len(cont_tokens)]
            ...
            for i, tok in enumerate(cont_tokens):
                pos = cont_start + i - 1  # -1 because we predict next token
                if pos >= 0 and pos < log_probs.shape[0]:
                    total_log_prob += log_probs[pos, tok].item()
            
            # Check if max prob prediction matches
...
def evaluate_blimp(model, tokenizer, device, limit=100):
    """Run BLiMP evaluation using lm_eval."""
    if not LM_EVAL_AVAILABLE:
        print("Skipping BLiMP - lm_eval not installed")
        return {}
    
    model.eval()
    
    # Wrap model for lm_eval
    lm = LMEvalWrapper(model, tokenizer, batch_size=1, device=device)
    ...
    try:
        ...
        if 'results' in results:
            for task, metrics in results['results'].items():
                if 'acc' in metrics:
                    blimp_results[task] = metrics['acc']
                elif 'acc,none' in metrics:
                    blimp_results[task] = metrics['acc,none']
        
        return blimp_results
    
    except Exception as e:
        print(f"BLiMP evaluation error: {e}")
        return {'error': str(e)}
...
def run_ablation(config: TrainingConfig, model_type: str) -> Dict[str, Any]:
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        # BLiMP evaluation
        if config.eval_blimp:
            print(f"\n  Running BLiMP evaluation...")
            blimp_results = evaluate_blimp(
                model, tokenizer, config.device,
                limit=config.blimp_limit
            )
            epoch_results['blimp'] = blimp_results
            
            if blimp_results and 'error' not in blimp_results:
                # Compute average
                avg_blimp = sum(blimp_results.values()) / len(blimp_results) if blimp_results else 0
                print(f"  BLiMP Avg Accuracy: {avg_blimp:.4f}")
...
def verify_pipeline(config: TrainingConfig):
    ...
    for model_type in ['holographic', 'swiglu']:
        ...
        # Test BLiMP (quick)
        if LM_EVAL_AVAILABLE and config.eval_blimp:
            print("  Testing BLiMP evaluation (limit=5)...")
            blimp_results = evaluate_blimp(model, tokenizer, config.device, limit=5)
            if blimp_results and 'error' not in blimp_results:
                print(f"  ✓ BLiMP works: {len(blimp_results)} tasks evaluated")
            else:
                print(f"  ⚠ BLiMP issue: {blimp_results}")
...
def main():
    ...
    parser.add_argument('--no-blimp', action='store_true', help='Skip BLiMP evaluation')
    parser.add_argument('--blimp-limit', type=int, default=100, help='BLiMP samples per task')
    ...
    config = TrainingConfig(
        ...
        eval_blimp=not args.no_blimp,
        blimp_limit=args.blimp_limit,
    )
    ...
    for model_type, results in all_results.items():
        ...
        # Final BLiMP
        if results['epochs'] and results['epochs'][-1].get('blimp'):
            blimp = results['epochs'][-1]['blimp']
            if blimp and 'error' not in blimp:
                avg = sum(blimp.values()) / len(blimp)
                print(f"  Final BLiMP Avg: {avg:.4f}")
